...
함수를 호출하면 함수의 본문에 지정된 특정 코드가 실행된다. 즉, 함수 호출로 원하는 코드를 원하는 시점에 실행시킬 수 있다. 흐름제어란 원하는 코드를 원하는 시점에 실행시키는 것이다. 여기서 흐름제어를 위해 함수를 사용할 수 있을 것이라는 아이디어를 떠올릴 수 있다.

# 예시

예시로 while을 반복구문 없이 구현해 보겠다. 예시에서 쓰일 언어는 Javascript이다.
